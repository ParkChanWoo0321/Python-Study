# -*- coding: utf-8 -*-
"""
Created on Sat Dec 14 14:44:08 2024
...
class Claw(pygame.sprite.Sprite):
    def __init__(self,image,position):
        ...
        self.angle_speed=2.5 # 집게의 각도 변경 폭 (좌푸 이동 속도)
...
class Gemstone(pygame.sprite.Sprite):
    def __init__(self,image,position,price,speed):
        super().__init__()
        self.image=image
        self.rect=image.get_rect(center=position) # 중심점 표시
        self.price=price
        self.speed=speed
...
def setup_gemstone():
    ...
    # 작은 금
    small_gold=Gemstone(gemstone_images[0],(200,380),small_gold_price,small_gold_speed) # 0 번째 이미지를 (200,380) 위치에 
...
# 가로위치는 화면 가로 기준으로 절반, 세로 크기는 위에서 110 픽셀
